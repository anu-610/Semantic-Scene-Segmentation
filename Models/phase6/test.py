...
OUTPUT_DIR = os.path.join(current_script_path, "Submission_Results_Phase6")

# --- MODEL PATH (FAILSAFE) ---
MODEL_PATH = model_utils.get_model_path(
    "best_model_phase6_fpn.pth", 
    "/kaggle/input/models/devil610/phase6/pytorch/default/1/best_model_phase6_fpn.pth"
)
...
# ==========================================
# 5. MODEL SETUP (FPN with ResNeXt50)
# ==========================================


print(f"⏳ Loading FPN (ResNeXt50) from {MODEL_PATH}...")
model = smp.FPN(
...
# 6. INFERENCE LOOP
# ==========================================
print("🚀 Generating Full Phase 6 Report...")
results = []
...
for img_name in tqdm(test_files):
    ...
    with torch.no_grad():
        logits = model(aug)
        # Resize prediction back to match Ground Truth
    ...
    # Subsampling for Confusion Matrix
...
with open(f"{OUTPUT_DIR}/evaluation_metrics.txt", "w") as f:
    f.write(f"PHASE 6 (FPN) OFFICIAL MEAN IoU: {avg_iou:.4f}\n\nPer-Class Breakdown:\n")
    class_avgs = []
    for i in range(10):
...
plt.xticks(rotation=45, ha='right'); plt.title(f'Phase 6 IoU (Mean: {avg_iou:.2f})'); plt.ylim(0, 1.0); plt.tight_layout()
...
plt.title('Phase 6 Confusion Matrix'); plt.tight_layout(); plt.savefig(f"{OUTPUT_DIR}/confusion_matrix.png")
...
print(f"🏆 PHASE 6 COMPLETE! Final Mean IoU: {avg_iou:.4f}")